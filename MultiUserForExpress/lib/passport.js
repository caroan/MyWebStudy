    ...
    passport.use(
        new LocalStrategy(//로컬 전략의 구체적 방식을 정의
            ...
            function(username, password, done){
                ...
                //console.log(user);
                if(user){
                    bcrypt.compare(password, user.password, function(err, result){ //입력받은 비번, 암호화된 비번, 콜백함수를 호출
                        if(result === true){ //비번이 맞으면 result값은 참을 반환한다.
                            return done(null, user, { message: '환영합니다.'});
                        }else{
                            return done(null, false, {message: '해당 비밀 번호가 틀렸습니다.'});
                        }
                    });
                }
                else{
                    return done(null, false, {message: '해당 유저가 없습니다.'});
                }
